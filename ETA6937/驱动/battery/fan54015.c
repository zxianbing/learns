...
#define ISAFE1350 6
#define ISAFE1450 7
#define ISAFE 7  //1.95A
#define VSAFE 12 //4.44V
...
static void fan54015_set_value(BYTE reg, BYTE reg_bit,BYTE reg_shift, BYTE val)
{
	...
	printf("######fan54015write_reg reg  = %d value =%d/%x\n",reg, tmp, tmp);
}
...
void sprdchg_fan54015_ic_init(void)
{
	...
	fan54015_sw_reset();
	//reg 6	
	fan54015_set_value(FAN5405_REG_SAFETY, FAN5405_VSAFE,FAN5405_VSAFE_SHIFT, VSAFE);  // VSAFE = 4.44V
	fan54015_set_value(FAN5405_REG_SAFETY, FAN5405_ISAFE, FAN5405_ISAFE_SHIFT, ISAFE);	// ISAFE = 195mA (68mOhm)
	//reg 1
...
void sprdchg_fan54015_init(void)
{
	...
	if ((vendor_id != VENDOR_FAN54015)
	    && (vendor_id != VENDOR_TQ24157)&& (vendor_id != VENDOR_PSC5415)) {
